      ...
      public function scopeStatut($query, $statut)
{
    return $query->where(function ($q) use ($statut) {
        // Utilisation de COALESCE pour gérer les valeurs nulles dans PostgreSQL
        $q->whereRaw('
            dettes.montant - (SELECT COALESCE(SUM(montant), 0) FROM paiements WHERE paiements.dette_id = dettes.id)
        ' . ($statut == 'solde' ? '= 0' : '> 0'));
    });
}
